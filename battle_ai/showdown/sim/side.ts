...
export class Side {
	...
	chooseMove(
		...
		targetLoc = 0,
		megaDynaOrZ: "mega" | "zmove" | "ultra" | "dynamax" | "" = ""
	) {
		...
		const maxMove =
			megaDynaOrZ === "dynamax" || pokemon.volatiles["dynamax"]
		...
		if (dynamax && (this.choice.dynamax || !canDynamax)) {
			if (pokemon.volatiles["dynamax"]) {
				...
				if (this.battle.gen < 8) {
					return this.emitChoiceError(
						`Can't move: Dynamaxing doesn't exist before Gen 8.`
					);
